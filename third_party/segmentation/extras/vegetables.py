...
SIZE_VEGETABLES = np.array([0.055, 0.1, 0.08])


def segment_vegetables(
    ...
    mask_vegetables = (35 < img_h) & (img_h < 50)
    mask_vegetables &= (60 < img_s) & (img_s < 200)
    mask_vegetables &= (100 < img_v) & (img_v < 245)

    mask_vegetables &= ~mask_others

    # mask_vegetables_color = mask_vegetables

    mask_vegetables = extract_largest_contour(mask_vegetables)

    ds = img_depth[mask_vegetables]
    if len(ds) == 0:
        return mask_vegetables, None
    d_min = ds.min()  # - 30
    d_max = ds.max()  # + 20
    mask_depth = (d_min <= img_depth) & (img_depth <= d_max)

    idx_points = point_cloud.flatten_indices(mask_vegetables)
    ...
    mask_vegetables = mask_depth & mask_xyz & ~mask_others

    mask_vegetables = extract_largest_contour(mask_vegetables)

    # Get box pose.
    pos = xyzs.mean(axis=0)
    pos[2] = xyz_min[2] + SIZE_VEGETABLES[2] / 2
    ...
    # cv2.imshow("s", img_s)
    # cv2.imshow("v", img_v)
    # cv2.imshow("color", mask_vegetables_color.astype(np.uint8) * 255)
    # cv2.imshow("depth", mask_depth.astype(np.uint8) * 255)
    # cv2.imshow("xyz", mask_xyz.astype(np.uint8) * 255)
    # cv2.imshow("vegetables", mask_vegetables.astype(np.uint8) * 255)
    # key = cv2.waitKey(0)
    # if key is not None and chr(key) == "q":
    #     exit(1)

    return mask_vegetables, ObjectInfo(pos, quat, SIZE_VEGETABLES)
