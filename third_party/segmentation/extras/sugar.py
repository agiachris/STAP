...
SIZE_SUGAR = np.array([0.04, 0.09, 0.17])
DEBUG = False
...
def segment_sugar(
    ...
    mask_sugar = (20 < img_h) & (img_h < 40)
    mask_sugar &= (120 < img_s) & (img_s < 230)
    mask_sugar &= (120 < img_v) & (img_v < 220)
    mask_sugar &= ~mask_others

    if DEBUG:
        mask_sugar_color = mask_sugar

    mask_sugar = extract_largest_contour(mask_sugar)

    ds = img_depth[mask_sugar]
    if len(ds) == 0:
        return mask_sugar, None
    d_min = ds.min() - 60
    d_max = ds.max() + 20
    mask_depth = (d_min <= img_depth) & (img_depth <= d_max)

    idx_points = point_cloud.flatten_indices(mask_sugar)
    xyzs = point_cloud.points[idx_points]
    xyz_min = xyzs.min(axis=0) - np.array([0.02, 0, 0])
    xyz_max = xyzs.max(axis=0) + np.array([0.02, 0, 0.08])
    ...
    mask_sugar = mask_depth & mask_xyz

    mask_sugar = extract_largest_contour(mask_sugar)

    # Get box pose.
    pos = xyzs.mean(axis=0)
    pos[2] = xyz_min[2] + SIZE_SUGAR[2] / 2
    ...
    if DEBUG:
        ...
        cv2.imshow("s", img_s)
        cv2.imshow("v", img_v)
        cv2.imshow("color", mask_sugar_color.astype(np.uint8) * 255)
        cv2.imshow("depth", mask_depth.astype(np.uint8) * 255)
        cv2.imshow("xyz", mask_xyz.astype(np.uint8) * 255)
        cv2.imshow("sugar", mask_sugar.astype(np.uint8) * 255)
        key = cv2.waitKey(0)
        if key is not None and chr(key) == "q":
            exit(1)

    return mask_sugar, ObjectInfo(pos, quat, SIZE_SUGAR)
