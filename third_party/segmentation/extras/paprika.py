...
SIZE_PAPRIKA = np.array([0.055, 0.1, 0.08])
...
def segment_paprika(
    ...
    mask_paprika = (15 < img_h) & (img_h < 60)
    mask_paprika &= (30 < img_s) & (img_s < 255)
    mask_paprika &= (30 < img_v) & (img_v < 100)
    mask_paprika &= ~mask_others

    # mask_paprika_color = mask_paprika

    # mask_paprika = extract_largest_contour(mask_paprika)
    #
    # ds = img_depth[mask_paprika]
    # if len(ds) == 0:
    #     return mask_paprika, None
    # d_min = ds.min()  # - 30
    # d_max = ds.max()  # + 20
    # mask_depth = (d_min <= img_depth) & (img_depth <= d_max)
    #
    # idx_points = point_cloud.flatten_indices(mask_paprika)
    ...
    # xyz_min[0] = max(xyz_min[0], xyz_max[0] - SIZE_PAPRIKA[1])
    # xyz_min[2] = xyz_max[2] - SIZE_PAPRIKA[2] - 0.01
    ...
    #
    # mask_paprika = mask_depth & mask_xyz & ~mask_others
    #
    # mask_paprika = extract_largest_contour(mask_paprika)
    #
    # # Get box pose.
    # pos = xyzs.mean(axis=0)
    # pos[2] = xyz_min[2] + SIZE_PAPRIKA[2] / 2
    #
    ...
    cv2.imshow("s", img_s)
    cv2.imshow("v", img_v)
    # cv2.imshow("color", mask_paprika_color.astype(np.uint8) * 255)
    # cv2.imshow("depth", mask_depth.astype(np.uint8) * 255)
    # cv2.imshow("xyz", mask_xyz.astype(np.uint8) * 255)
    cv2.imshow("paprika", mask_paprika.astype(np.uint8) * 255)
    key = cv2.waitKey(0)
    if key is not None and chr(key) == "q":
        exit(1)

    return mask_paprika, ObjectInfo(pos, quat, SIZE_PAPRIKA)
