...
SIZE_FUDGE = np.array([0.02, 0.07, 0.16])
...
def segment_fudge(
    ...
    mask_fudge = (10 < img_h) & (img_h < 35)
    mask_fudge &= (20 < img_s) & (img_s < 200)
    mask_fudge &= (20 < img_v) & (img_v < 90)
    mask_fudge &= ~mask_others

    # mask_fudge_color = mask_fudge

    mask_fudge = extract_largest_contour(mask_fudge)

    ds = img_depth[mask_fudge]
    if len(ds) == 0:
        return mask_fudge, None
    d_min = ds.min()  # - 30
    d_max = ds.max()  # + 20
    mask_depth = (d_min <= img_depth) & (img_depth <= d_max)

    idx_points = point_cloud.flatten_indices(mask_fudge)
    ...
    xyz_min[0] = max(xyz_min[0], xyz_max[0] - SIZE_FUDGE[1])
    ...
    mask_fudge = mask_depth & mask_xyz & ~mask_others

    mask_fudge = extract_largest_contour(mask_fudge)

    # Get box pose.
    pos = xyzs.mean(axis=0)
    pos[2] = xyz_min[2] + SIZE_FUDGE[2] / 2
    ...
    # cv2.imshow("s", img_s)
    # cv2.imshow("v", img_v)
    # cv2.imshow("color", mask_fudge_color.astype(np.uint8) * 255)
    # cv2.imshow("depth", mask_depth.astype(np.uint8) * 255)
    # cv2.imshow("xyz", mask_xyz.astype(np.uint8) * 255)
    # cv2.imshow("fudge", mask_fudge.astype(np.uint8) * 255)
    # key = cv2.waitKey(0)
    # if key is not None and chr(key) == "q":
    #     exit(1)

    return mask_fudge, ObjectInfo(pos, quat, SIZE_FUDGE)
