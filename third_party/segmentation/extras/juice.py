...
SIZE_JUICE = np.array([0.055, 0.1, 0.08])
...
def segment_juice(
    ...
    mask_juice = (15 < img_h) & (img_h < 35)
    mask_juice &= (20 < img_s) & (img_s < 210)
    mask_juice &= (100 < img_v) & (img_v < 245)

    mask_juice &= ~mask_others

    # mask_juice_color = mask_juice

    mask_juice = extract_largest_contour(mask_juice)

    ds = img_depth[mask_juice]
    if len(ds) == 0:
        return mask_juice, None
    d_min = ds.min() - 10
    d_max = ds.max()  # + 20
    mask_depth = (d_min <= img_depth) & (img_depth <= d_max)

    idx_points = point_cloud.flatten_indices(mask_juice)
    ...
    mask_juice = mask_depth & mask_xyz & ~mask_others

    mask_juice = extract_largest_contour(mask_juice)

    # Get box pose.
    pos = xyzs.mean(axis=0)
    pos[2] = xyz_min[2] + SIZE_JUICE[2] / 2
    ...
    # cv2.imshow("s", img_s)
    # cv2.imshow("v", img_v)
    # cv2.imshow("color", mask_juice_color.astype(np.uint8) * 255)
    # cv2.imshow("depth", mask_depth.astype(np.uint8) * 255)
    # cv2.imshow("xyz", mask_xyz.astype(np.uint8) * 255)
    # cv2.imshow("juice", mask_juice.astype(np.uint8) * 255)
    # key = cv2.waitKey(0)
    # if key is not None and chr(key) == "q":
    #     exit(1)

    return mask_juice, ObjectInfo(pos, quat, SIZE_JUICE)
