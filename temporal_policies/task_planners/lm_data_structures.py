...
class InContextExample:
    ...
    action_predicted: Optional[str] = None  # action predicted can be generated via scoring
    ...
    def explanation_prompt(self) -> str:
        ...
        return res + (
            f"{SCENE_OBJECT_PROMPT}{self.scene_objects}\n"
            f"{SCENE_OBJECT_RELATIONSHIP_PROMPT}{self.scene_object_relationships}\n" if self.scene.get("relationships") else ""
            f"{HUMAN_INSTRUCTION_PROMPT}{self.human}\n"
    ...
    def overall_example(self) -> str:
        ...
        if self.use_scene_object_relationships and self.scene_object_relationships is not None:
            res += f"{SCENE_OBJECT_RELATIONSHIP_PROMPT}{self.scene_object_relationships}\n"
...
class Result:
    ...
    test_robot: bool = False  # whether to compare LM's robot's action sequence with the "ground truth"
