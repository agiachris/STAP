...
class UniformSamplingPlanner(Box2DPlannerBase):
    ...
    def _uniform_rollout(self, idx, env, branches, state=None): 
        ...
        if use_learned_dynamics:
            if state is None: curr_state = self._encode_state(idx, curr_state)
            else: curr_state = state
        ...
        if not success.max():
            if idx != self._idx: return getattr(np, self._agg_mode)(curr_returns)
        ...
        for opt_idx in opt_vars:
            ...
            if not use_learned_dynamics: next_envs = self._load_env(opt_idx, curr_envs)
            else: next_envs = [self._load_env(opt_idx, curr_envs)] * len(success)
            for i, (next_env, next_state) in enumerate(zip(next_envs, next_states)):
                next_returns[i] = self._critic_over_action_space(opt_idx, next_env, state=next_state, agg=True)
        ...
        for sim_dict in sim_vars:
            ...
            next_returns = np.zeros_like(curr_returns)            
            if not use_learned_dynamics: next_envs = self._load_env(sim_idx, curr_envs)
            else: next_envs = [self._load_env(sim_idx, curr_envs)] * len(success)
            for i, (next_env, next_state) in enumerate(zip(next_envs, next_states)):
                next_returns[i] = self._uniform_rollout(sim_idx, next_env, sim_branches, state=next_state)
            curr_returns = getattr(np, self._mode)((curr_returns, next_returns), axis=0)
        
        if idx != self._idx: return getattr(np, self._agg_mode)(curr_returns).item()
        return curr_actions[curr_returns.argmax()]
    ...
    def _critic_over_action_space(self, idx, env, state=None, agg=True):
        ...
        if agg: returns = getattr(np, self._agg_mode)(returns)
        return returns
