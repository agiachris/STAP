...
class IOManager(TaskManager):
    ...
    def _use_policy_actor(self, idx):
        ...
        use_policy = actor == "policy" or (actor is None and self._default_actor == "policy")
        return use_policy
    ...
    def _actor_interface(self, idx, **kwargs):
        ...
        if self._use_policy_actor(idx):
            ...
            if "variance" in opt_kwargs: actions = self._sample_policy(idx, **req_kwargs, **opt_kwargs)
            else: actions = self._policy(idx, **req_kwargs, **opt_kwargs)
        else:
            # Query random action
    ...
    def _use_learned_dynamics(self, idx):
        ...
        use_learned = model == "learned" or (model is None and self._default_model == "learned")
        return use_learned
    ...
    def _simulate_interface(self, idx, **kwargs):
        ...
        if self._use_learned_dynamics(idx):
            ...
            states, actions, is_np, is_batched = self._pre_process_states_actions(**req_kwargs, **opt_kwargs)
    ...
    def _pre_process_inputs(self, inputs):
        ...
        if is_np: inputs = utils.to_tensor(inputs).to(self._device)
        if not is_batched: inputs = utils.unsqueeze(inputs, 0)
    ...
    def _post_process_outputs(outputs, is_np, is_batched):
        ...
        """
        if not is_batched: outputs = utils.squeeze(outputs, 0)
        if is_np: outputs = utils.to_np(outputs)
        return outputs
    ...
    def _pre_process_states_actions(self, states, actions):
        ...
        if num_a == num_s: return states, actions, is_np, is_batched       
        if num_s < num_a and num_s == 1: states = states.tile(num_a, 1)
        elif num_s > num_a and num_a == 1: actions = actions.tile(num_s, 1)
        else: raise ValueError("Cannot match uneven number of states to actions")
    ...
    def _q_value_fn(self, idx, states, actions):
        """Compute Q(s, a).
        """
        states, actions, is_np, is_batched = self._pre_process_states_actions(states, actions)
    ...
    def _random_policy(self, envs, samples=1):
        ...
        if samples == 1: return envs.action_space.sample()
    ...
    def _sample_policy(self, idx, states, variance, samples=1, bounds=[-1, 1], **kwargs):
