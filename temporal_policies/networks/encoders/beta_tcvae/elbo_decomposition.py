...
def estimate_entropies(qz_samples, qz_params, q_dist):
    ...
    qz_samples = qz_samples.index_select(
        1, torch.autograd.Variable(torch.randperm(qz_samples.size(1))[:10000].cuda())
    )
    ...
    pbar = tqdm.tqdm(total=S)
    k = 0
    while k < S:
        batch_size = min(10, S - k)
        logqz_i = q_dist.log_density(
            qz_samples.view(1, K, S).expand(N, K, S)[:, :, k : k + batch_size],
            qz_params.view(N, K, 1, nparams).expand(N, K, S, nparams)[
                :, :, k : k + batch_size
            ],
        )
        k += batch_size
        ...
        marginal_entropies += (
            math.log(N) - logsumexp(logqz_i, dim=0, keepdim=False).data
        ).sum(1)
        # computes - log q(z) summed over minibatch
        logqz = logqz_i.sum(1)  # (N, S)
        joint_entropy += (
            math.log(N) - logsumexp(logqz, dim=0, keepdim=False).data
        ).sum(0)
...
def analytical_NLL(qz_params, q_dist, prior_dist, qz_samples=None):
    ...
    --------
        ...
        nlogpz (K,) Variable
    """
    pz_params = torch.autograd.Variable(
        torch.zeros(1).type_as(qz_params.data).expand(qz_params.size()), volatile=True
    )
...
def elbo_decomposition(vae, dataset_loader):
    ...
    for xs in dataset_loader:
        ...
        xs = torch.autograd.Variable(xs.view(batch_size, -1, 64, 64).cuda(), volatile=True)
        ...
        for _ in range(S):
            ...
            logpx += (
                vae.x_dist.log_density(xs, params=x_params)
                .view(batch_size, -1)
                .data.sum()
            )
    # Reconstruction term
    ...
    qz_params = torch.autograd.Variable(qz_params.cuda(), volatile=True)
    ...
    marginal_entropies, joint_entropy = estimate_entropies(
        qz_samples, qz_params, vae.q_dist
    )
    ...
    if hasattr(vae.q_dist, "NLL"):
        ...
        nlogqz_condx = -vae.q_dist.log_density(
            qz_samples, qz_params_expanded.transpose(0, 1).contiguous().view(K, N * S)
        ).mean(1)
