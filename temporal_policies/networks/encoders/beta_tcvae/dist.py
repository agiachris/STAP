...
class Normal(torch.nn.Module):
    ...
    def __init__(self, mu=0, sigma=1):
        ...
        self.normalization = torch.autograd.Variable(torch.Tensor([np.log(2 * np.pi)]))

        self.mu = torch.autograd.Variable(torch.Tensor([mu]))
        self.logsigma = torch.autograd.Variable(torch.Tensor([math.log(sigma)]))
    ...
    def sample(self, size=None, params=None):
        ...
        std_z = torch.autograd.Variable(torch.randn(mu.size()).type_as(mu.data))
...
class Laplace(torch.nn.Module):
    ...
    def __init__(self, mu=0, scale=1):
        ...
        self.normalization = torch.autograd.Variable(torch.Tensor([-math.log(2)]))

        self.mu = torch.autograd.Variable(torch.Tensor([mu]))
        self.logscale = torch.autograd.Variable(torch.Tensor([math.log(scale)]))
    ...
    def sample(self, size=None, params=None):
        ...
        # Unif(-0.5, 0.5)
        u = torch.autograd.Variable(torch.rand(mu.size()).type_as(mu.data)) - 0.5
...
class Bernoulli(torch.nn.Module):
    ...
    def __init__(self, p=0.5, stgradient=False):
        ...
        p = torch.Tensor([p])
        self.p = torch.autograd.Variable(torch.log(p / (1 - p) + eps))
    ...
    def log_density(self, sample, params=None):
        ...
        p = (torch.nn.functional.sigmoid(presigm_ps) + eps) * (1 - 2 * eps)
