...
from stap import agents, dynamics, encoders, envs, trainers
from stap import scod as scod_regression
from stap.dynamics import load as load_dynamics
from stap.dynamics import LatentDynamics, load_policy_checkpoints
...
class TrainerFactory(configs.Factory):
    ...
    def __init__(
        ...
        if issubclass(self.cls, trainers.AgentTrainer):
            if agent is None:
                ...
                ckpt_agent = agents.load(
                    checkpoint=checkpoint, env_kwargs=env_kwargs, device=device
                )
        ...
        elif issubclass(self.cls, trainers.PolicyTrainer):
            if agent is None:
                if checkpoint is None:
                    raise ValueError("Either agent or checkpoint must be specified")
                ckpt_agent = agents.load(
