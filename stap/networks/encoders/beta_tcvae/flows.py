...
class FactorialNormalizingFlow(torch.nn.Module):
    def __init__(self, dim, nsteps):
        super(FactorialNormalizingFlow, self).__init__()
        self.dim = dim
        self.nsteps = nsteps
        self.x_dist = dist.Normal()
        self.scale = torch.nn.Parameter(torch.Tensor(self.nsteps, self.dim))
        self.weight = torch.nn.Parameter(torch.Tensor(self.nsteps, self.dim))
        self.bias = torch.nn.Parameter(torch.Tensor(self.nsteps, self.dim))
    ...
    def log_density(self, y, params=None):
        ...
        x = y
        logdetgrad = torch.autograd.Variable(torch.zeros(y.size()).type_as(y.data))
        for i in range(self.nsteps):
            u = self.scale[i][None]
            w = self.weight[i][None]
            b = self.bias[i][None]
            act = torch.functional.tanh(x * w + b)
            x = x + u * act
