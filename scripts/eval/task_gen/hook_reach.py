...
def get_random_indices(boxes: Dict[int, str]) -> List[int]:
    arg_indices = list(range(len(boxes)))
...
def annotate_plan_skeleton(
    vars: List[Tuple[str, str]], plan_skeleton: List[str]
) -> List[str]:
    annotated_plan_skeleton = []
    for primitive in plan_skeleton:
        for v in vars:
            primitive = primitive.replace(v[0], v[1])
        annotated_plan_skeleton.append(primitive)
    return annotated_plan_skeleton
...
def main() -> None:
    ...
    while task_idx < num_tasks:
        while arg_indices:
            ...
            print(
                f"Plan skeleton: {annotate_plan_skeleton(vars, lifted_task['plan_skeleton'])}"
            )
