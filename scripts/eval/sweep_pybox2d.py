...
if __name__ == "__main__":
    ...
    parser.add_argument("--sweep-config", "-s", type=str, required=True, help="Path to parameter sweep config")
    parser.add_argument("--checkpoints", "-c", nargs="+", type=str, required=True, help="Path to model checkpoints")
    parser.add_argument("--path", "-p", type=str, required=True, help="Path to directory for saving results")
    parser.add_argument("--num-eps", "-n", type=int, default=100, help="Number of episodes to unit test across")
    parser.add_argument("--device", "-d", type=str, default="auto")
    args = parser.parse_args()

    # Load exec and sweep configs
    with open(args.exec_config, "r") as fs: exec_config = yaml.safe_load(fs)
    with open(args.sweep_config, "r") as fs: sweep_config = json.load(fs)
    ...
    for i, params in enumerate(sweep_params):
        ...
        with tempfile.NamedTemporaryFile(mode="w+") as ts:
            ...
            cmd += ["--path", fpath + f"_{i}.json", "--num-eps", str(args.num_eps), "--device", args.device]
