...
def plot_planning_results(
    ...
    def barplot(
        ...
        **kwargs,
        ...
        sns.barplot(ax=ax, data=df_plans[idx_subgoal], palette=palette, **kwargs)
        sns.barplot(ax=ax, data=df_plans[idx_ground_truth], palette=palette, **kwargs)
        sns.barplot(ax=ax, data=df_plans[idx_predicted], palette=palette, **kwargs)
        ax.set_xticklabels(
        ...
        for idx_bar, (bar, line) in enumerate(zip(ax.patches, ax.lines)):
            ...
            if idx_success_type == 2 and idx_class != num_classes - 1:
                ...
                bar.set_linestyle(":")
                if idx_class < 1:
                    bar.set_y(ground_truth_bars[idx_class].get_height())
                    bar.set_height(0)
                else:
    ...
    ax.set_title("Hook Reach/Constrained Packing: Task 1")
    ax.set_ylabel("")

    ax = axes[1]
    sns.barplot(ax=ax, data=df_plans, x="Method", y="Planning Time", palette=palette)
    ax.set_ylabel("")
    ax.set_yscale("log")
...
path = "../../plots/20220914/official/ablation_experiment"
envs = ["hook_reach/task0", "constrained_packing/task0"]
methods = [
