...
if __name__ == "__main__":
    ...
    assert (
        isinstance(args.gpus, list) or args.gpus is None
    ), "GPUs must be a list of ints or None."
    ...
    for i, (entry_point, script_ags) in enumerate(jobs):
        job_cpus = ",".join(
            [str(c) for c in cpu_list[i * cores_per_job : (i + 1) * cores_per_job]]
        )
        command_list = ["taskset", "-c", job_cpus, "python", entry_point]
        ...
        if job_gpus is not None:
            ...
            ] = job_gpus  # TODO: this doesn't support multi-gpu
        else:
            env = None
